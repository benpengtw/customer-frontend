  ...
  return (
    <Box pt={1}>
      <Paper style={{ maxWidth: theme.breakpoints.values[maxWidth] }}>
        ...
        {content && (
          <DialogContent classes={helpPadding ? { root: classes.helpPadding } : null}>{content}</DialogContent>
        )}
        {actions && (
          <Box pb={2} pr={2}>
            <DialogActions classes={{ action: fullWidthActions ? classes.fullWidth : null }}>{actions}</DialogActions>
          </Box>
        )}
      </Paper>
    </Box>
